...
class jg_input_fuse_segmentation_batch_layer(caffe.Layer):
    ...
    def load_data_view(self):
        ...
        for i in range(int(self.batch_size)):
            if self.patches_processed>=len(self.views):
                self.needToChangeImage = True
...
class jg_input_fuse_segmentation_layer(caffe.Layer):
    ...
    def load_img_data(self, idx,data_=False):
        ...
        #in_ -= self.mean_bgr
            ...
            print "Error : shape not accepted."
        if self.dummy_data:
            return np.ones((3,128,128),dtype=np.float32)
        else:
            return in_
        self.batch_data = self.load_img_data(self.iter_counter)[np.newaxis, ...]
    ...
    def load_img_segmentation(self, idx,delta=False):
        ...
        if self.dummy_data:
            return np.ones((1,128,128),dtype=np.float32)
        else:
            return segmentation
...
class jg_rebatching_layer(caffe.Layer):
    ...
    def rebatch_features(self):
        ...
        newbatch = np.zeros([256, 3, 480, 1280])

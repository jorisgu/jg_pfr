...
class jg_input_shrec_var_layer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.variation = np.zeros(self.nb_key_frame)
        if self.var>0:
            print("Variation activated.", file=self.logfile)
            self.variation = np.random.randint(-self.var,self.var,self.nb_key_frame)
    ...
    def load_data(self):
        for i in range(int(self.batch_size)):
            if self.images_processed>=self.nb_images:
                ...
                self.nb_epochs += 1
                if self.shuffle:
                    print("Shuffling.", file=self.logfile)
                    self.permutation = np.random.permutation(self.nb_images)
            if self.var>0:
                self.variation = np.random.randint(-self.var,self.var,self.nb_key_frame)
            idx = self.permutation[self.images_processed]
            ...
            previous_depth_id = 0
            for j in range(self.nb_key_frame):
                # borned key frame + variation + monotonous growth
                depth_id = int(max(previous_depth_id,min(max(int(math.floor(self.list_images[idx,6]*(1+j)/(self.nb_key_frame+1)))+self.variation[j],0),self.list_images[idx,6]-1)))
                depth_img = Image.open(sequence_folder_path+str(depth_id)+'_depth.png')
                depth_img = depth_img.resize((self.data_h,self.data_w),Image.ANTIALIAS)
                data[j,...]=depth_img   #depths[int(math.floor(self.nb_images*(1+j)/(self.nb_key_frame+1))),:]
...
class jg_input_shrec_var_biclass_layer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # self.batch_label = np.zeros((int(self.batch_size),1,self.num_classes), dtype=np.uint8)
        self.batch_label = np.zeros((int(self.batch_size)), dtype=np.uint8)
        self.batch_label_bi = np.zeros((int(self.batch_size)), dtype=np.uint8)
    ...
    def forward(self, bottom, top):
        top[0].data[...] = self.batch_data
        top[1].data[...] = self.batch_label
        top[2].data[...] = self.batch_label_bi
        self.batches_processed += 1
    ...
    def reshape(self, bottom, top):
        ...
        top[0].reshape(*self.batch_data.shape)
        top[1].reshape(*self.batch_label.shape)
        top[2].reshape(*self.batch_label_bi.shape)
    ...
    def load_data(self):
        for i in range(int(self.batch_size)):
            ...
            label = int(self.list_images[idx,4])-1
            label_bi = int(self.list_images[idx,1])-1



            self.batch_data[i,...] = data
            self.batch_label[i,...] = label
            self.batch_label_bi[i,...] = label_bi

            self.images_processed+=1

class jg_input_shrec_14bi28_layer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.batch_label = np.zeros((int(self.batch_size)), dtype=np.uint8)
        self.batch_label_bi = np.zeros((int(self.batch_size)), dtype=np.uint8)
        self.batch_label_28 = np.zeros((int(self.batch_size)), dtype=np.uint8)
    ...
    def forward(self, bottom, top):
        ...
        top[1].data[...] = self.batch_label
        top[2].data[...] = self.batch_label_bi
        top[3].data[...] = self.batch_label_28
        self.batches_processed += 1
    ...
    def reshape(self, bottom, top):
        ...
        top[1].reshape(*self.batch_label.shape)
        top[2].reshape(*self.batch_label_bi.shape)
        top[3].reshape(*self.batch_label_28.shape)
    ...
    def load_data(self):
        for i in range(int(self.batch_size)):
            ...
            label = int(self.list_images[idx,4])-1
            label_bi = int(self.list_images[idx,1])-1
            label_28 = int(self.list_images[idx,5])-1
            ...
            self.batch_label[i,...] = label
            self.batch_label_bi[i,...] = label_bi
            self.batch_label_28[i,...] = label_28
